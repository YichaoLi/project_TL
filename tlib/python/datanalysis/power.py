...
def pk(d, boxsize=1200.,bin2fact=1./16., filename='',getnuminbin=False,overwrite=True,
...
def covar2d(d, maxorder1d=2, maxorder2=None, outroot='', setmean0=True,dividebyvar=False,calcvar=False,boxsize=939.):
    phase1, phase2 = 1./16., 5./16.
    s = d.shape
    if (setmean0):
        d -= N.mean(d.flatten())

    k,pmean,numinbin = pk(d[:,:,0],getnuminbin=True)
    powerarray = N.zeros((s[0],len(k)))
    if (calcvar):
        vararray = N.zeros(s[0])
    for xi in N.arange(0,s[0]):
        d2d=1.*d[xi,:,:]
        if (calcvar):
            vararray[xi] = N.var(d2d.flatten())
            if (dividebyvar):
                d2d /= N.sqrt(vararray[xi])
        k,powerarray[xi,:] = pk(d2d,boxsize=boxsize)

                    
    N.savetxt(outroot+'.p',N.transpose([k,pmean,numinbin]),fmt='%e')
    N.savetxt(outroot+'.hrs',powerarray,fmt='%e')
    if (calcvar):
        N.savetxt(outroot+'.var',vararray,fmt='%e')


def weightings(d, maxorder1d=2, maxorder2=None, outroot='', setmean0=True,include000=False,boxsize=500.,dividebyvar=False,calcvar=False):
    phase1, phase2 = 1./16., 5./16.
    s = d.shape
    if (setmean0):
        d -= N.mean(d.flatten())
    ...
    if (calcvar):
        vararray = N.zeros(((2*maxorder1d+1)**3-1+int(include000))*2)
            ...
            for zi in N.arange(-maxorder1d,maxorder1d+1):
                if include000 or ((xi!=0) or (yi!=0) or (zi!=0)):
                    ...
                    for phase in [1./16., 5./16.]:
                        weightedd = d*sqrt2*N.cos(2.*N.pi*(xi*x+yi*y+zi*z)/N.float(s[0]) + phase)
                        if (calcvar):
                            vararray[numspectra] = N.var(weightedd.flatten())
                            if (dividebyvar):
                                weightedd /= N.sqrt(vararray[numspectra])
                        k,powerarray[numspectra,:] = pk(weightedd,boxsize=boxsize)
                        numspectra += 1
